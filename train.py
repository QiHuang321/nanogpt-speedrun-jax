...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.01  # handicap
    ...
    muon_ns_iters_warmup_steps: int = 500  # ramp NS iters up to muon_ns_iters
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # dynamic-bound loop rather than a Python `range`.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # adaptive Newton-Schulz iteration count: fewer iters early in training,
        # ramping linearly up to ns_iters over ns_iters_warmup_steps.
