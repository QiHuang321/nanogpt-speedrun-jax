...
class Logger:
    def __init__(self):
        ...
        # Gradient-norm diagnostic: standalone JSON file, separate from the
        # text log and wandb. Purely observational; never read back into
        # training.
        self.gradnorm_file = f"logs/{self.run_id}_gradnorm.json"
        self.gradnorm_records = []
    ...
    def log_gradnorm(self, record: dict):
        # Diagnostic hook: append one gradient-norm record and persist the
        # running list to its own JSON file. Master-only and side-effect free
        # with respect to training (does not touch prev_metrics / wandb).
        if not self.is_master:
            return
        self.gradnorm_records.append(record)
        with open(self.gradnorm_file, "w") as f:
            json.dump(self.gradnorm_records, f)
...
class Config:
    ...
    # Extra, purely-observational evaluation points expressed as fractions of
    # the run (in addition to the val_loss_every schedule and the final eval).
    # These only log val_loss; they never alter params/opt_state or trigger
    # early stop, so the training path is left unchanged.
    ...
    # muon for matrices
    muon_base_lr: float = 0.0225  # handicap
    ...
    adam_nonmat_base_lr: float = 0.035555555555555556  # handicap
...
def train_step(
    ...
    # Gradient-norm diagnostic (read-only): global L2 norm of the same
    # micro-batch-averaged gradients fed to the optimizer above. Accumulated
    # in float32 to avoid bf16 over/underflow. Does not affect the update.
    grad_norm = jnp.sqrt(
...
def train_loop(config: Config):
    ...
    with mesh:
        ...
        # Precompute extra intermediate evaluation steps (observational only).
        # Restricted to strictly inside (0, n_train_iters) so we never duplicate
        # the final eval or evaluate at step 0; steps that coincide with the
        # regular val_loss_every schedule are handled there (see the elif below).
        ...
        for step in range(config.n_train_iters):
            ...
            if step > 0 and (step % config.val_loss_every == 0):
                ...
                # Intermediate, purely observational evaluation: logs one extra
                # val_loss sample point without mutating training state or
                # stopping early. The elif also prevents a double eval when an
                # intermediate step coincides with the val_loss_every schedule.
                run_evaluation(
                    step,
                    config,
                    params,
