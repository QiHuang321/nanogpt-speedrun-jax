...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped from muon_ns_iters_init (early in
    # training) up to muon_ns_iters (final), so early steps run fewer iterations.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced int (adaptive schedule), so loop dynamically.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters_init (fewer, early in training) up to ns_iters (final).
        progress = jnp.clip(step / n_train_iters, 0.0, 1.0)
        ns_steps = ns_iters_init + jnp.floor(
            progress * (ns_iters - ns_iters_init + 1)
        ).astype(jnp.int32)
        ns_steps = jnp.clip(ns_steps, ns_iters_init, ns_iters)
