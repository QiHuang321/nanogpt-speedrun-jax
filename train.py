...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token embedding (wte); no separate
    # lm_head parameter is allocated. The output projection in gpt_forward uses
    # params["wte"].T. (To untie, restore: params["lm_head"] = sharded_zeros(
    # (config.d_model, config.vocab_size)) and use params["lm_head"] below.)
