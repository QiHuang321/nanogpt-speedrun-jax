...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied output projection: the lm_head shares the token-embedding weights
    # (logits use wte.T in gpt_forward), so no separate lm_head parameter is
    # allocated. The adam_lm_head optimizer simply ends up with no params.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: reuse the token embedding (transposed) as the lm_head.
