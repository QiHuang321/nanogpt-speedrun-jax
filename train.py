...
class Logger:
    def __init__(self):
        ...
        # from logs/ and records/) so it doesn't interfere with the main logs.
        self.grad_norms = []
        self.gradnorm_file = f"gradnorm_{self.run_id}.json"
    ...
    def log_grad_norm(self, step: int, grad_norm: float):
        # Diagnostic-only hook: record the global gradient norm to a standalone
        # JSON file. Does not affect training or the main metric logs.
        if not self.is_master:
            return
...
class Config:
    ...
    muon_ns_iters: int = 5  # final/max Newton-Schulz iterations
    muon_ns_iters_init: int = 2  # fewer NS iterations early in training
    muon_ns_iters_warmup_steps: int = 500  # ramp ns_iters_init -> ns_iters
    muon_eps: float = 1e-7
...
def get_ns_iters(it, ns_iters_init, ns_iters_final, warmup_steps):
    # ramp Newton-Schulz iteration count from init -> final over warmup_steps
    frac = jnp.minimum(it / warmup_steps, 1.0)
    iters = ns_iters_init + frac * (ns_iters_final - ns_iters_init)
...
def muon(
    ...
    def update(grads, params, state):
        ...
        cur_ns_iters = get_ns_iters(
            step, ns_iters_init, ns_iters, ns_iters_warmup_steps
        )
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, cur_ns_iters, eps).astype(
                    p.dtype
                )
...
def train_step(
    ...
    # Diagnostic-only: global L2 norm of the (averaged) gradient used for the
    # update. Computed in float32; does not alter the grads passed to the
    # optimizer, so training is unaffected.
    grad_norm = jnp.sqrt(
        sum(
