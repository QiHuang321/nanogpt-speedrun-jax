...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the token embedding (wte) is reused as the lm_head output
    # projection (see gpt_forward), so no separate lm_head parameter is created.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding (vocab, d_model) transposed as the
    # output projection (d_model, vocab) instead of a separate lm_head matrix.
