...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: no separate lm_head parameter — the output projection reuses
    # the token embedding matrix (transposed) in gpt_forward.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: reuse the token embedding (vocab, d_model) transposed to
    # (d_model, vocab) as the output projection, sharing one set of weights.
