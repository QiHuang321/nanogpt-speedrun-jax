...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. The return value is a
    # multiplier on each optimizer's base (peak) LR, so the peak is unchanged:
    #   1) linear warmup from 0 -> 1 over the first n_warmup_iters steps,
    #   2) a stable plateau held at the peak (multiplier 1.0), then
