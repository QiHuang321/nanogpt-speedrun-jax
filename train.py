...
class Config:
    ...
    muon_ns_iters: int = 2
    # Adaptive Newton-Schulz: ramp the NS iteration count from muon_ns_iters_min
    # early in training up to muon_ns_iters, linearly over muon_ns_warmup_steps.
    # Fewer iterations early (when grads are noisy and the orthogonalization need
    # not be precise) saves compute; later steps use the full count.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop rather than a Python range() over a static count.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # training, ramping linearly from ns_iters_min up to ns_iters over the
        # first ns_warmup_steps steps.
        ns_frac = jnp.minimum(step / ns_warmup_steps, 1.0)
        cur_ns_iters = ns_iters_min + jnp.floor(
            ns_frac * (ns_iters - ns_iters_min) + 0.5
        ).astype(jnp.int32)
        new_m = tree_map(
