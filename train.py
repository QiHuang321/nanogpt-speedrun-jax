...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # reuses wte.T in gpt_forward, so there is no separate lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token embedding weights (wte) as the
    # lm_head by transposing (vocab_size, d_model) -> (d_model, vocab_size).
