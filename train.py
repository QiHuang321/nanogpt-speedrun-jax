...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.050625  # handicap: quartered from tuned 0.04 (s0 headroom), then +50% repeatedly
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    muon_ns_iters: int = 5  # max Newton-Schulz iters (reached at end of training)
    muon_ns_iters_min: int = 3  # min Newton-Schulz iters (used early in training)
    muon_eps: float = 1e-7

    # adam for non-matrices
    adam_nonmat_base_lr: float = 0.0118519
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz: ramp iters linearly from ns_iters_min (early)
        # up to ns_iters (max) over the course of training. Fewer iters early
        # are cheaper and adequate while gradients are coarse.
        progress = jnp.clip(step / n_train_iters, 0.0, 1.0)
        ns_iters_step = jnp.round(
            ns_iters_min + progress * (ns_iters - ns_iters_min)
        ).astype(jnp.int32)
