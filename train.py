...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.015
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    # Newton-Schulz iteration count is adaptive to the training step: it ramps
    # from muon_ns_iters_start early in training (a coarse orthogonalization is
    # fine while gradients are noisy and changing fast, and it's cheaper) up to
    # muon_ns_iters, reaching the final count by muon_ns_iters_warmup_steps.
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. The peak multiplier is 1.0, so
    # the peak LR equals each optimizer's base_lr (unchanged):
    #   - warmup: linear ramp 0 -> 1 over the first n_warmup_iters steps
    #   - stable: constant 1.0
    #   - decay:  linear ramp 1 -> 0 over the final n_warmdown_iters steps
    # jnp.maximum(..., 1) guards against divide-by-zero when a phase is disabled
    # (length 0); the corresponding branch is never selected in that case.
    warmup_lr = (it + 1) / jnp.maximum(n_warmup_iters, 1)
    decay_lr = (n_train_iters - it) / jnp.maximum(n_warmdown_iters, 1)
    lr = jnp.where(
        it < n_warmup_iters,
        warmup_lr,
        jnp.where(it < n_train_iters - n_warmdown_iters, 1.0, decay_lr),
    )
    return lr
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (per-training-step) scalar, so iterate with a
        # fori_loop rather than a Python range, which would require a static
        # bound. The coefficients are constant across iterations.
        def _body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters_start to ns_iters over the first ns_iters_warmup_steps,
        # rounded to an integer number of iterations (fewer early).
        ns_frac = jnp.minimum(step / ns_iters_warmup_steps, 1.0)
        ns_steps = jnp.round(
            ns_iters_start + ns_frac * (ns_iters - ns_iters_start)
        ).astype(jnp.int32)
        new_m = tree_map(
