...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # (wte) rather than being a separate "lm_head" parameter. gpt_forward uses
    # wte.T for the head, so a single weight feeds both the embedding lookup and
    # the logits and autodiff accumulates gradients from both into wte.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    # lm_head by transposing it to (d_model, vocab_size).
