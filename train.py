...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding (wte): gpt_forward reuses
    # params["wte"].T as the output projection, so there is no separate param.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied to token embedding (wte)
    c = config.logit_softcap  # tanh-style logit softcap (c=15)
