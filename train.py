...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding (see gpt_forward), so no separate
    # output projection parameter is allocated here.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # weight tying: reuse the (vocab, d_model) embedding as the output projection
