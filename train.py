...
class Config:
    ...
    muon_ns_warmup_steps: int = 500  # ramp ns iters from init to full over these steps
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (per-step adaptive) count, so use fori_loop
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # use fewer Newton-Schulz iterations early in training, ramping linearly
        # from ns_iters_init up to the full ns_iters over ns_warmup_steps.
