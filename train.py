...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embedding/unembedding: the lm_head reuses wte^T (see gpt_forward), so
    # no separate output-projection weight is allocated here.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied embedding/unembedding: project with wte^T instead of a separate lm_head.
