...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    params = dict()
    # Tied weights: the token embedding (wte) also serves as the lm_head output
    # projection (used transposed in gpt_forward), so there is no separate
    # lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding (transposed) as the output projection.
