...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # hold the peak constant through the stable phase, then linearly decay back
    # down to zero over the final n_warmdown_iters steps. The peak multiplier
    # stays 1.0 so the peak LR (base_lr) is unchanged.
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    # QK-norm: RMSNorm the queries and keys over the head dimension before RoPE.
    q = rms_norm(q, config)
    k = rms_norm(k, config)
