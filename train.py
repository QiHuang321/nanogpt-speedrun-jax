...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iterations are ramped from muon_ns_iters_init (early in
    # training) up to muon_ns_iters (final), i.e. fewer iterations early.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced scalar (adaptive schedule); fori_loop handles
        # both static Python ints and dynamic bounds.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: ramp linearly with training
        # progress from ns_iters_init (fewer, early) up to ns_iters (final).
        progress = jnp.minimum(step / n_train_iters, 1.0)
        cur_ns_iters = jnp.maximum(
            ns_iters_init,
            jnp.round(
                ns_iters_init + progress * (ns_iters - ns_iters_init)
            ).astype(jnp.int32),
        )
        new_m = tree_map(
