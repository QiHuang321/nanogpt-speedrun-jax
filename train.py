...
class Config:
    ...
    # iterations early (cheaper, coarser orthogonalization) climbing to the
    # full muon_ns_iters once past muon_ns_warmup_steps. muon_ns_iters_min is
    # the count used at step 0.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # dynamic-bound fori_loop rather than a Python range loop.
        def _ns_step(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters_min at step 0 up to ns_iters once step >= ns_warmup_steps.
