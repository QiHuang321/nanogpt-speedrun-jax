...
class Config:
    ...
    muon_ns_iters_warmup_steps: int = 500  # ramp ns iters from min -> full over this many steps
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced scalar (adaptive over training step), so use a
        # dynamic loop rather than Python-level unrolling.
        def _body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: fewer iters early, ramping
        # linearly from ns_iters_min up to ns_iters over ns_iters_warmup_steps.
        ns_frac = jnp.minimum(step / ns_iters_warmup_steps, 1.0)
        ns_steps = ns_iters_min + jnp.floor(
