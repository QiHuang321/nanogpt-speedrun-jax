...
class Config:
    ...
    muon_ns_iters: int = 2
    # Adaptive Newton-Schulz: use fewer NS iterations early in training (when the
    # orthogonalization need not be precise) and ramp up to muon_ns_iters.
    muon_ns_iters_start: int = 1
    muon_ns_warmup_steps: int = 300
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced scalar (adaptive schedule), so use a dynamic
        # fori_loop rather than a Python range() that requires a static count.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # linearly from ns_iters_start up to ns_iters over ns_warmup_steps steps.
        ns_frac = jnp.minimum(step / jnp.maximum(ns_warmup_steps, 1), 1.0)
        cur_ns_iters = jnp.round(
