...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses the token embedding (wte)
    # transposed (see gpt_forward), so no separate lm_head parameter is created.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token embedding (wte) transposed as the
    # lm_head weight, so embedding and unembedding share a single parameter.
