...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    #   * stable at the peak (1.0) through the middle of training,
    #   * linear decay 1 -> 0 over the final n_warmdown_iters steps.
    # max(..., 1) guards the denominators when a phase is disabled (length 0);
    # the corresponding branch is then never selected by jnp.where.
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    q = rms_norm(q, config)  # QK-norm: RMSNorm over the per-head dim before RoPE
    k = rms_norm(k, config)
