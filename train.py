...
class Config:
    ...
    n_warmup_iters: int = 50  # warmup-stable-decay: linear warmup phase
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection (lm_head) reuses the token-embedding
    # matrix (wte) instead of allocating a separate parameter. See gpt_forward,
    # which transposes wte for the final logit projection.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token-embedding matrix (wte) as the
    # lm_head weights. wte is (vocab_size, d_model); transpose to
    # (d_model, vocab_size) for the linear logit projection.
