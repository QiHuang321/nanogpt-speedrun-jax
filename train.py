...
class Logger:
    def __init__(self):
        ...
        # from the text log and from logs/ and records/ so it never interferes
        # with training or the existing log outputs.
    ...
    def log_grad_norm(self, step: int, grad_norm: float):
        # Diagnostic-only hook: append {step, grad_norm} and rewrite the JSON
        # file. Independent of log()/flush() so it adds no training overhead and
        # leaves the existing text/wandb metrics untouched.
        if not self.is_master:
            return
...
def train_step(
    ...
    # Diagnostic only: global L2 norm of the gradients actually fed to the
    # optimizer. Surfaced via metrics for a separate JSON log; it does not
    # feed back into the update, so training dynamics are unchanged.
    grad_norm = jnp.sqrt(
...
def train_loop(config: Config):
    ...
    with mesh:
        ...
        for step in range(config.n_train_iters):
            ...
            # Gradient-norm diagnostic: pulled from the same replicated scalar
            # shard as the loss and written to its own JSON file. Purely
            # observational — does not affect params, opt_state, or logging.
