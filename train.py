...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # base LR: linear warmup to the peak (1.0), a constant "stable" phase held at
    # the peak, then a linear decay from the peak down to zero over the final
    # n_warmdown_iters steps. Peak LR is unchanged (base_lr * 1.0).
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embeddings: the lm_head reuses the (transposed) token embedding
    # instead of owning a separate weight, so there is a single shared matrix.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: project with the transposed token embedding (wte) rather
    # than a separate output matrix.
