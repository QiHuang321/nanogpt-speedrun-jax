...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped with the training step: start at
    # `muon_ns_iters_start` (cheaper, coarser orthogonalization while early
    # updates are noisy) and rise to the full `muon_ns_iters` over
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-training-step) value, so iterate with a
        # dynamic loop instead of Python-level unrolling.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp the Newton-Schulz iteration count from ns_iters_start up to the
        # full ns_iters over ns_iters_warmup_steps (fewer iterations early).
        ns_frac = jnp.minimum(step / ns_iters_warmup_steps, 1.0)
        ns_iters_dyn = jnp.round(
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, ns_iters_dyn, eps).astype(p.dtype)
