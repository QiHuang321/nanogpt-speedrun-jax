...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # applied to each optimizer's base LR, so the peak LR is unchanged:
    #   - warmup: linearly ramp 0 -> 1 over the first n_warmup_iters steps
    #   - stable: hold at the peak (1.0)
    #   - decay:  linearly anneal 1 -> 0 over the final n_warmdown_iters steps
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    params = dict()
    # Tied weights: the token embedding doubles as the output projection
    # (logits = x @ wte.T), so there is no separate lm_head parameter.
