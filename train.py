...
class Config:
    ...
    # Warmup-stable-decay (trapezoid) schedule: linear warmup, a constant plateau
    # at peak LR, then a linear decay over the final f_warmdown_iters fraction of
    # training. Peak LR is unchanged; only the trailing decay leg is enabled.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Newton-Schulz iteration count ramps with training progress: fewer
        # iterations early (noisy gradients make exact orthogonalization wasteful)
        # up to the full count later in training.
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embedding / lm_head: the output projection reuses wte (transposed)
    # in gpt_forward rather than owning a separate weight matrix, so there is no
    # params["lm_head"] leaf. The adam_lm_head optimizer is left in place but
    # becomes inert (no leaf is labeled "lm_head").
