...
class Config:
    ...
    n_warmup_iters: int = 50  # warmup phase of the trapezoid LR schedule
    f_warmdown_iters: float = 0.2  # handicap; decay phase of the trapezoid LR schedule
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # peak (base) LR. Three phases:
    #   1. warmup : linearly ramp from 0 up to the peak over the first
    #               n_warmup_iters steps.
    #   2. stable : hold flat at the peak (multiplier 1.0) -- peak LR unchanged.
    #   3. decay  : linearly ramp from the peak back down to 0 over the final
