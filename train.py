...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses the token embedding matrix
    # (lm_head == wte.T, applied in gpt_forward), so there is no separate
    # lm_head parameter. Its gradient is accumulated into wte automatically.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the (transposed) token embedding matrix as
    # the lm_head weights instead of a separate parameter.
