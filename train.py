...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        # `steps` may be a traced scalar (the NS iteration count is made adaptive
        # to the training step in `muon`), so use a dynamic-trip-count fori_loop
        # rather than a Python-unrolled `for i in range(steps)`.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: use fewer NS iterations early
        # (orthogonalization need not be precise while grads are noisy) and ramp
        # linearly up to the full `ns_iters` over `ns_iters_warmup_steps`. This
        # is a traced scalar, so the NS loop runs as a dynamic-trip fori_loop.
