...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. Peak LR (the 1.0 multiplier on
    # base_lr) is unchanged from the previous schedule.
    #   1) linear warmup from 0 -> 1 over the first n_warmup_iters steps
    #   2) stable: constant peak (1.0) through the middle of training
...
def mlp_forward(params, x):
    ...
    x = jnp.square(relu(x))  # relu^2 (swapped from gelu)
