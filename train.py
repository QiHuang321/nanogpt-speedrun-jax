...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding (wte) matrix rather
    # than being a separate parameter, so no "lm_head" leaf is initialized; the
    # output projection in gpt_forward uses wte.T directly.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied lm_head: reuse the wte embedding
