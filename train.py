...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses wte (see gpt_forward), so there
    # is no separate lm_head parameter. Gradients from both the embedding lookup
    # and the output logits accumulate into wte, optimized by adam_embed.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: lm_head is wte transposed (no separate parameter).
