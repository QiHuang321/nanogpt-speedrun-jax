...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # (params["wte"]) instead of owning a separate (d_model, vocab_size)
    # parameter, so there is no standalone "lm_head" leaf. See gpt_forward.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: project with the transpose of the token-embedding matrix
    # (params["wte"] is (vocab_size, d_model)) rather than a separate lm_head.
