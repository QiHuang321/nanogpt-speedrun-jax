...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. Peak multiplier stays 1.0, so
    # the peak LR (base_lr * 1.0) is unchanged.
    #   1) linear warmup from 0 -> 1 over the first n_warmup_iters steps
    #   2) hold at the peak (1.0) through the stable phase
