...
class Config:
    ...
    muon_ns_iters: int = 5
    # adaptive Newton-Schulz: ramp from muon_ns_iters_min up to muon_ns_iters,
    # using fewer iterations early (when gradients are noisy) to save compute.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Newton-Schulz iteration count grows from ns_iters_min to ns_iters over
        # the first ns_warmup_steps, then stays constant (fewer iters early).
        ns_progress = jnp.minimum(step / ns_warmup_steps, 1.0)
        ns_iters_now = ns_iters_min + jnp.floor(
            ns_progress * (ns_iters - ns_iters_min)
        ).astype(jnp.int32)
        new_m = tree_map(
