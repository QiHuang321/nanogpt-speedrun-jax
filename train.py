...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding matrix (applied as
    # wte^T at the output), so there is no separate lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: project with the transposed token-embedding matrix.
