...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token embedding (wte.T), so no
    # separate lm_head parameter is created.
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    # QK-norm: RMSNorm queries and keys over the per-head dim before RoPE.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding as the output projection.
