...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule, normalized so the peak is 1.0
    # (the unchanged peak LR is each optimizer's base_lr * 1.0):
    #   - linear warmup from 0 -> 1 over the first n_warmup_iters steps,
    #   - hold at the peak of 1.0 during the stable phase,
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-step adaptive) value, so use fori_loop
        # rather than a Python range loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: use fewer iterations early in
        # training (gradients are noisy and don't need precise orthogonalization)
        # and ramp up to the configured maximum as training progresses.
        ns_frac = jnp.minimum(step / n_train_iters, 1.0)
        ns_steps = (
            ns_iters_min + jnp.round(ns_frac * (ns_iters - ns_iters_min))
        )
        ns_steps = jnp.clip(ns_steps, ns_iters_min, ns_iters).astype(jnp.int32)
        new_m = tree_map(
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding: the unembedding reuses wte
    # (transposed) in gpt_forward, so no separate lm_head parameter is created.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied unembedding: reuse the token embedding matrix (wte) transposed as the
    # lm_head weight instead of a separate parameter.
