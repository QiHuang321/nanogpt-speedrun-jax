...
class Config:
    ...
    muon_ns_iters: int = 2  # handicap (max NS iters, reached late in training)
    muon_ns_iters_start: int = 1  # fewer NS iters early in training
    muon_ns_iters_warmup_frac: float = 0.5  # frac of training to ramp NS iters up
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` is traced (it varies with the training step), so use a
        # dynamic-bound fori_loop (lowers to a while_loop) rather than a Python
        # range loop, which would require a static iteration count.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Newton-Schulz iteration count ramps up with the training step: fewer
        # iters early (cheaper, coarser orthogonalization), reaching the full
        # `ns_iters` by `ns_iters_warmup_frac` of the way through training.
        ns_warmup_steps = jnp.maximum(1.0, ns_iters_warmup_frac * n_train_iters)
        ns_progress = jnp.minimum(1.0, step / ns_warmup_steps)
        ns_iters_now = ns_iters_start + jnp.floor(
            ns_progress * (ns_iters - ns_iters_start)
        ).astype(jnp.int32)
        ns_iters_now = jnp.maximum(ns_iters_now, 1)
