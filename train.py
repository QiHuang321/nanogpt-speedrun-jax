...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses the token embedding (wte) — see
    # gpt_forward, which applies params["wte"].T as lm_head. No separate param,
    # so the adam_lm_head optimizer simply operates on an empty subset (no-op).
