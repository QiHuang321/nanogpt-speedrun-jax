...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: fewer early in training,
        # ramping linearly up to ns_iters as the run progresses.
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: lm_head shares the token-embedding matrix (wte). There is no
    # separate lm_head parameter; the output projection uses wte.T in gpt_forward.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    logits = 2 * c * jax.nn.sigmoid(logits / (c / 2))
