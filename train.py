...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head reuses the token embedding (wte.T) instead of a
    # separate matrix, so no standalone "lm_head" parameter is created here.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: project with the transpose of the token embedding.
