...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule, returned as a multiplier of the
    # peak (base) LR. Peak is unchanged: the stable plateau multiplier is 1.0.
    #   - warmup: linearly ramp 0 -> 1 over the first n_warmup_iters steps
    #   - stable: hold at the peak (1.0) through the middle of training
    #   - decay:  linearly ramp 1 -> 0 over the final n_warmdown_iters steps
    # The jnp.maximum(..., 1) guards keep the expression well-defined when a
    # phase is configured to zero length (that phase is then never selected).
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embeddings: the output projection reuses wte (transposed) rather than
    # a dedicated lm_head matrix, so there is a single shared weight that gets
    # gradients from both the embedding lookup and the logit projection.
    # See gpt_forward for the tied use.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    # instead of a separate lm_head parameter.
