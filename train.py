...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced int32 (adaptive per training step), so use
        # fori_loop instead of a Python loop.
        def _body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # fewer Newton-Schulz iterations early in training, ramping to the
        # full count over the same window as the momentum warmup
        ns_iters_now = jnp.round(
            ns_iters_init + frac * (ns_iters - ns_iters_init)
        ).astype(jnp.int32)
