...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # (unchanged) peak LR: linear warmup 0 -> 1 over n_warmup_iters, a stable
    # plateau at 1, then a linear decay 1 -> 0 over the final n_warmdown_iters.
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # (wte), so there is no separate lm_head parameter. See gpt_forward, which
    # applies wte.T as the unembedding. wte is optimized by adam_embed and
    # receives gradients from both the embedding lookup and the unembedding.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied unembedding: reuse wte (vocab, d_model) transposed to (d_model, vocab).
