...
class Config:
    ...
    n_warmup_iters: int = 50  # ~3% linear warmup for the trapezoid schedule
    f_warmdown_iters: float = 0.2  # final 20% linearly decays peak LR -> 0
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # plateau sits at 1.0 — peak LR (= base_lr) is therefore unchanged:
    #   1) linear warmup 0 -> 1 over the first n_warmup_iters steps
    #   2) constant 1.0 through the stable phase
