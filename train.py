...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # applied on top of each optimizer's base (peak) LR, so the peak LR is left
    # unchanged: linear warmup 0 -> 1, a stable plateau at the peak (1.0), then
    # a linear decay 1 -> 0 over the final n_warmdown_iters steps. Denominators
    # are floored at 1 so a zero-length warmup/decay degrades to no phase.
    warmup_denom = jnp.maximum(n_warmup_iters, 1)
    warmdown_denom = jnp.maximum(n_warmdown_iters, 1)
    warmup_lr = (it + 1) / warmup_denom
    stable_lr = 1.0
    decay_lr = (n_train_iters - it) / warmdown_denom
    lr = jnp.where(
        it < n_warmup_iters,
        warmup_lr,
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: lm_head shares the token embedding (see gpt_forward, which
    # uses params["wte"].T as the output projection), so no separate lm_head
    # weight is allocated. The adam_lm_head optimizer then matches no params and
    # becomes a no-op.
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    q = rms_norm(q, config)  # QK-norm
    k = rms_norm(k, config)  # QK-norm
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: reuse the token embedding (transposed) as the output
    # projection instead of a separate lm_head weight. wte is (vocab, d_model);
    # wte.T is (d_model, vocab) as linear() expects.
