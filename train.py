...
class Config:
    ...
    muon_ns_iters: int = 2  # handicap; final (max) Newton-Schulz iters
    muon_ns_iters_init: int = 1  # Newton-Schulz iters early in training
    muon_eps: float = 1e-7
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. The peak multiplier stays 1.0,
    # so the peak LR equals each optimizer's base_lr, unchanged from before:
    #   - linear warmup from 0 -> 1 over the first n_warmup_iters steps
    #   - stable (constant) plateau at the peak through the middle of training
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (step-dependent) int; use fori_loop so the
        # trip count can vary at run time. With a dynamic bound this lowers
        # to a while loop under jit.
        def body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: cheaper (fewer iters) early in
        # training, ramping up to ns_iters by the end.
        ns_frac = jnp.minimum(step / n_train_iters, 1.0)
        dyn_ns_iters = ns_iters_init + jnp.floor(
            ns_frac * (ns_iters - ns_iters_init) + 0.5
        ).astype(jnp.int32)
        dyn_ns_iters = jnp.clip(dyn_ns_iters, ns_iters_init, ns_iters)
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, dyn_ns_iters, eps).astype(
                    p.dtype
                )
