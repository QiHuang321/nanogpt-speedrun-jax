...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses wte (transposed) instead of a
    # separate lm_head parameter — see gpt_forward. wte is (vocab, d_model);
    # the head consumes its transpose (d_model, vocab).
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token embedding (transposed) as the
