...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped with the training step: fewer
    # iterations early (orthogonalization need not be precise while grads are
    # large/noisy) up to muon_ns_iters by muon_ns_warmup_steps.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-training-step) value, so use a JAX loop
        # with a dynamic bound rather than a Python-unrolled range.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: ramp from ns_iters_init to
        # ns_iters over the first ns_warmup_steps (fewer iterations early).
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the lm_head shares the token-embedding matrix (wte) rather
    # than being a separate parameter; logits are computed as x @ wte.T in
    # gpt_forward. (To untie, restore a dedicated params["lm_head"] here.)
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied weights: lm_head == wte.T
