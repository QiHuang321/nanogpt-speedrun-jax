...
class Config:
    ...
    # Warmup-stable-decay (trapezoid) LR schedule. Peak LR (the stable plateau)
    # is unchanged; only the early warmup ramp and the final linear decay are
    # added. n_warmdown_iters is derived from f_warmdown_iters in __post_init__.
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped from muon_ns_iters_init up to
    # muon_ns_iters over the first muon_ns_warmup_steps steps. Early gradients are
    # noisy, so fewer iterations early saves compute without hurting the update.
    muon_ns_iters_init: int = 1
    muon_ns_warmup_steps: int = 500
    muon_eps: float = 1e-7

    # adam for non-matrices
    adam_nonmat_base_lr: float = 0.08
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # sits at 1.0. It is multiplied by each optimizer's base_lr downstream, so
    # the peak LR is unchanged from a plain constant schedule.
    #   1) linear warmup:  0 -> 1 over the first n_warmup_iters steps
    #   2) stable plateau: 1.0 (peak) until the decay window begins
    #   3) linear decay:   1 -> 0 over the final n_warmdown_iters steps
    # jnp.maximum(..., 1) guards the zero-warmup / zero-warmdown cases, in which
    # the corresponding branch is never selected and the schedule reduces to a
    # constant 1.0.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced int32 (adaptive to the training step), so use a
        # JAX loop rather than a Python range.
        def _body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp the Newton-Schulz iteration count from ns_iters_init up to ns_iters
        # over the first ns_warmup_steps steps (fewer iterations early).
        ns_frac = jnp.minimum(step / jnp.maximum(ns_warmup_steps, 1), 1.0)
        ns_count = jnp.round(
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, ns_count, eps).astype(p.dtype)
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the output projection reuses the token embedding (wte.T in
    # gpt_forward) instead of a separate lm_head parameter, so no lm_head leaf
    # is allocated. To untie, restore:
    #     params["lm_head"] = sharded_zeros((config.d_model, config.vocab_size))
    # and revert the gpt_forward logits line to linear(x, params["lm_head"]).
