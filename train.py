...
class Config:
    ...
    n_warmup_iters: int = 50  # warmup-stable-decay: linear warmup over the first 50 steps
    ...
    muon_ns_iters: int = 2  # handicap; full Newton-Schulz iters, reached after ns warmup
    # Adaptive Newton-Schulz: use fewer NS iterations early in training (cheaper,
    # and a coarse orthogonalization is enough while the momentum is still noisy),
    # then ramp up to muon_ns_iters over the first muon_ns_warmup_steps steps.
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # optimizer's peak (base) LR so the peak LR is unchanged:
    #   1) linear warmup  0 -> 1  over the first n_warmup_iters steps
    #   2) stable         1       through the middle of training
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop (not a Python range loop) so `steps` may be a traced value,
        # letting the caller vary the iteration count per training step.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # training, ramping linearly up to ns_iters over the first
        # ns_warmup_steps steps (then held at ns_iters).
