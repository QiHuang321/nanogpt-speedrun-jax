...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embeddings: the output projection reuses the token-embedding matrix
    # (lm_head = wte^T), so there is no separate lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: project back through the token-embedding matrix (wte^T)
    # rather than a separate lm_head parameter.
