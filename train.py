...
class Config:
    ...
    muon_ns_iters: int = 5
    muon_ns_iters_init: int = 2  # fewer Newton-Schulz iters early; ramps up to muon_ns_iters
    muon_eps: float = 1e-7
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # applied to the (unchanged) peak/base LR:
    #   - warmup: linear ramp 0 -> 1 over the first n_warmup_iters steps
    #   - stable: hold at the peak (1.0)
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-training-step) value, so use a dynamic loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # linearly up to ns_iters over the course of training.
        ns_frac = jnp.minimum(step / jnp.maximum(n_train_iters, 1), 1.0)
