...
class Config:
    ...
    adam_nonmat_base_lr: float = 0.04 * 2 / 3 * 2 / 3 * 2 / 3 * 2 / 3 * 2 / 3
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # than being a separate parameter; gpt_forward projects logits with wte.T.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: reuse the token embedding (wte) as the output projection.
