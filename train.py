...
class Config:
    ...
    muon_ns_iters: int = 5
    muon_ns_iters_init: int = 2  # fewer Newton-Schulz iters early in training
    muon_eps: float = 1e-7
...
def get_ns_iters(it, ns_iters_init, ns_iters_final, n_train_iters):
    # Ramp the Newton-Schulz iteration count from `ns_iters_init` (fewer, early)
    # up to `ns_iters_final`, reaching the full count by the end of training.
    frac = jnp.minimum(it / n_train_iters, 1.0)
    n = ns_iters_init + frac * (ns_iters_final - ns_iters_init)
    return jnp.clip(jnp.round(n), ns_iters_init, ns_iters_final).astype(jnp.int32)
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (step-dependent) count, so use fori_loop
        # rather than a Python range() that would require a static value.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        ns_iters_step = get_ns_iters(step, ns_iters_init, ns_iters, n_train_iters)
