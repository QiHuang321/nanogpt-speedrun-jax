...
class Logger:
    def __init__(self):
        ...
        # Gradient-norm diagnostic: buffered in memory and flushed to a JSON
        # file kept separate from the normal logs/ and records/ outputs. This
        # is purely observational and does not affect the training trajectory.
        self.grad_norm_file = f"grad_norm_diagnostics_{self.run_id}.json"
        self.grad_norms = []
    ...
    def log_grad_norm(self, step: int, grad_norm: float):
        # Diagnostic-only hook: record the per-step global gradient L2 norm.
        # Buffered here and persisted via flush_grad_norms; never alters params,
        # optimizer state, RNG, or data consumption.
        if not self.is_master:
            return
    ...
    def flush_grad_norms(self):
        ...
        with open(self.grad_norm_file, "w") as f:
            json.dump(
                {"run_id": self.run_id, "grad_norms": self.grad_norms}, f, indent=2
            )
...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.0225  # handicap: quartered from tuned 0.04 (s0 headroom), then +50%
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    muon_ns_iters: int = 5  # full Newton-Schulz iteration count, reached after warmup
    muon_ns_iters_init: int = 2  # fewer Newton-Schulz iterations early in training
    muon_ns_warmup_steps: int = 300  # steps to ramp NS iters from init up to full
    muon_eps: float = 1e-7

    # adam for non-matrices
    adam_nonmat_base_lr: float = 0.0177778
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop (rather than a Python for) so `steps` may be a traced,
        # step-dependent value rather than a static iteration count.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Newton-Schulz iteration count ramps from ns_iters_init up to ns_iters
        # over ns_warmup_steps (fewer orthogonalization iterations early on).
        ns_iters_now = ns_iters_init + jnp.minimum(step, ns_warmup_steps) * (
            ns_iters - ns_iters_init
        ) // ns_warmup_steps
        new_m = tree_map(
...
def train_step(
    ...
    # Diagnostic only: global L2 norm of the gradient actually fed to the
    # optimizer. Computed in float32 for stability; does not modify final_grads
    # and therefore leaves the training update bit-for-bit unchanged.
    grad_norm = jnp.sqrt(
        sum(
    ...
    with mesh:
        ...
        for step in range(config.n_train_iters):
            ...
            if step > 0 and (step % config.val_loss_every == 0):
                ...
                # Periodically persist the grad-norm diagnostic so partial runs
                # still produce a JSON file; bounded IO (validation cadence).
                logger.flush_grad_norms()
                if (
