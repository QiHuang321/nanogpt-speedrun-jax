...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding (wte): no separate output
    # projection weight is allocated; gpt_forward reuses wte.T as the
    # unembedding, and autodiff accumulates gradients into wte from both uses.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied unembedding: reuse the token embedding (transposed) as lm_head.
