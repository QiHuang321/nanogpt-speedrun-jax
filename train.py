...
class Config:
    ...
    muon_ns_iters: int = 5
    # Newton-Schulz iteration count is ramped over training: start at
    # muon_ns_iters_init and rise linearly to the full muon_ns_iters over the
    # first muon_ns_iters_warmup_steps steps, then hold at the full count.
    # Fewer iterations early (when the momentum buffer is still noisy and exact
    # orthogonalization buys little) saves compute; the late-training update is
    # unchanged.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-training-step adaptive) value, so use a
        # rolled fori_loop rather than a Python range() that unrolls at trace
        # time and would require a static count.
        def _body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters over the first ns_iters_warmup_steps, then hold. Fewer NS
        # iterations early, the full count later.
        ns_frac = jnp.minimum(step / max(ns_iters_warmup_steps, 1), 1.0)
        ns_now = jnp.round(
            ns_iters_init + ns_frac * (ns_iters - ns_iters_init)
        ).astype(jnp.int32)
        ns_now = jnp.clip(ns_now, ns_iters_init, ns_iters)
