...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the lm_head shares the token-embedding matrix (gpt_forward
    # uses wte.T as the output projection), so there is no separate lm_head
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight-tied output projection: reuse the token embedding (wte) transposed
    # as the lm_head instead of a separate params["lm_head"] matrix.
