...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule: a linear warmup up to the peak,
    # a stable plateau held at the peak, then a linear decay down to zero. The
    # peak (the 1.0 multiplier applied to each optimizer's base_lr) is unchanged.
