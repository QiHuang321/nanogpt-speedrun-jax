...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding (wte): no separate output-projection
    # parameter is allocated; gpt_forward reuses wte.T as the logits weight.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    # wte is (vocab, d_model); linear() wants (d_model, vocab), hence wte.T.
