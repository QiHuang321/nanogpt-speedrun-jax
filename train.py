...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped with the training step: start
    # with muon_ns_iters_start iterations and linearly reach muon_ns_iters over
    # muon_ns_iters_warmup_steps steps (fewer, cheaper iterations early when the
    # momentum buffer is still noisy and approximate orthogonalization suffices).
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop (rather than a Python range loop) so that `steps` may be a
        # traced value: the iteration count is scheduled per training step.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp the Newton-Schulz iteration count from ns_iters_start up to
        ...
        ns_iters_now = ns_iters_start + jnp.floor(
            ns_frac * (ns_iters - ns_iters_start)
        ).astype(jnp.int32)
