...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    params = dict()
    # Tied embeddings: a single `wte` matrix serves as both the token embedding
    # and (transposed) the lm_head output projection, so there is no separate
    # lm_head parameter. The adam_lm_head optimizer simply has no leaves to act
    # on; multi_optimizer skips it.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied lm_head: reuse the embedding matrix
