...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head is not its own parameter; the output projection
    # reuses the token-embedding matrix (wte) transposed — see gpt_forward.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: project with the transpose of the token embedding (wte)
    # instead of a separate lm_head, so the input and output embeddings share
    # one set of weights (gradients from both paths accumulate into wte).
