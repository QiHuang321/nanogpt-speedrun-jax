...
class Config:
    ...
    # adam for embeddings
    adam_embed_base_lr: float = 0.3
    adam_embed_beta1: float = 0.9
    adam_embed_beta2: float = 0.95

    # adam for lm head
    adam_lm_head_base_lr: float = 0.004
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding matrix (wte), so no
    # separate lm_head parameter is allocated. gpt_forward projects with wte.T.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: reuse the token-embedding matrix (transposed) as the output
    # projection instead of a separate lm_head weight.
