...
class Config:
    ...
    muon_ns_iters: int = 2
    # Adaptive Newton-Schulz: use this many iterations early in training and
    # ramp linearly up to muon_ns_iters by the end (fewer iters early, when
    # the orthogonalization need not be precise on large/noisy grads).
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced scalar (adaptive iteration count), so use a
        # fori_loop with a dynamic bound rather than an unrolled Python loop.
        def _body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters_init at the start of training to ns_iters at the end, so we
        # spend fewer iterations early when precise orthogonalization matters
        # least.
        ns_progress = jnp.clip(step / n_train_iters, 0.0, 1.0)
        cur_ns_iters = ns_iters_init + jnp.round(
            ns_progress * (ns_iters - ns_iters_init)
        ).astype(jnp.int32)
        new_m = tree_map(
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head weights are tied to the token embedding (wte); no separate param.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: reuse the token embedding matrix (transposed) as the output projection.
