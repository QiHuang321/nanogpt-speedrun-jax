...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the lm_head output projection is not a separate parameter.
    # gpt_forward reuses the token-embedding matrix (wte.T) as the output
    # weight, so the single wte is trained by both the embedding lookup and the
    # output projection. (To untie, restore a standalone
    # params["lm_head"] = sharded_zeros((config.d_model, config.vocab_size))
    # and have gpt_forward use it again.)
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding (wte.T) as the output projection.
