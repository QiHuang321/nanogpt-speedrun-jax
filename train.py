...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # peak LR (which is left unchanged, i.e. the plateau multiplier is 1.0).
    # Linearly warm up to 1.0 over the first n_warmup_iters steps, hold at the
    # peak through the stable phase, then linearly decay back to 0 over the final
    # n_warmdown_iters steps. The jnp.maximum(..., 1) guards keep the unselected
    # branches finite when a phase is disabled (length 0).
