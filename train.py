...
class Config:
    ...
    muon_ns_warmup_steps: int = 500  # ramp NS iters init -> muon_ns_iters
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced scalar (adaptive per training step), so loop
        # with fori_loop rather than a Python-level unroll.
        def _ns_step(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters over the first ns_warmup_steps steps. Early in training the
        # gradients are noisy, so precise orthogonalization isn't worth the
        # extra matmuls; later we use the full iteration count.
        ns_frac = jnp.minimum(step / ns_warmup_steps, 1.0)
        cur_ns_iters = jnp.floor(
            ns_iters_init + ns_frac * (ns_iters - ns_iters_init) + 0.5
        ).astype(jnp.int32)
        new_m = tree_map(
