...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the token embedding doubles as the output projection
    # (lm_head = wte.T), so no separate lm_head parameter is allocated.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    # wte is (vocab, d_model); transpose to (d_model, vocab) for linear().
