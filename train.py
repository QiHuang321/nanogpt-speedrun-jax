...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection shares the token-embedding matrix
    # (wte) and is applied as wte.T in gpt_forward, so there is no separate
    # lm_head parameter. The adam_lm_head optimizer simply gets no leaves.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: project with the transpose of the token-embedding matrix.
