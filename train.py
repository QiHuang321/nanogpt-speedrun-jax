...
class Config:
    ...
    # Warmup-stable-decay (trapezoid) LR schedule: linear warmup to the peak,
    # a stable plateau at the peak, then a linear decay (see get_lr). Peak LR is
    # unchanged (the schedule multiplier still tops out at 1.0 == base_lr).
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding: the unembedding reuses wte.T in
    # gpt_forward (weight sharing), so there is no separate lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied unembedding: reuse the token-embedding matrix (wte) transposed.
