...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoidal) schedule: linear warmup up to the peak,
    # a stable plateau held at the peak, then a linear decay back down to zero.
    # The peak multiplier stays at 1.0 so the peak LR (base_lr) is unchanged.
