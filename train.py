...
class Config:
    ...
    muon_ns_iters: int = 5
    muon_ns_iters_min: int = 2  # fewer Newton-Schulz iters early in training
    muon_ns_iters_warmup_frac: float = 0.5  # ramp NS iters to full by this fraction of training
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (step-dependent) int, so use a dynamic loop
        # rather than a Python `range`.
        def body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: use fewer iterations early
        # (cheaper, and exact orthogonalization matters less on noisy early
        # gradients), ramping up to the full `ns_iters` by `ns_iters_warmup_frac`
        # of training.
        ns_frac = jnp.minimum(
            step / jnp.maximum(1.0, ns_iters_warmup_frac * n_train_iters), 1.0
        )
        ns_iters_now = jnp.maximum(
            1,
            jnp.round(ns_iters_min + ns_frac * (ns_iters - ns_iters_min)).astype(
                jnp.int32
            ),
        )
