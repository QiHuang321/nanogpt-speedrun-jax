...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.03375  # handicap: quartered from tuned 0.04 (s0 headroom), then +50% twice
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied output head: the lm_head shares the token-embedding matrix (used as
    # wte.T in gpt_forward), so there is no separate lm_head parameter. Gradients
    # from both the embedding lookup and the output projection accumulate into
    # wte, which is optimized by adam_embed.
