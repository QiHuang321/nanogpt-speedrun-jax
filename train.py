...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # gpt_forward, so there is no separate lm_head parameter. To untie, restore:
...
def mlp_forward(params, x):
    ...
    x = jnp.square(relu(x))  # gelu swapped back to relu^2
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: project with the transposed token embedding instead of a
    # separate lm_head. To untie, restore: linear(x, params["lm_head"]).
