...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding: there is no separate lm_head
    # parameter; the output projection reuses wte.T in gpt_forward.
...
def train_step(
    ...
    # Diagnostic-only global gradient norm (accumulated in float32 for
    # stability). This is a read-only reduction over the grads; it does not
    # feed back into the update below, so the training trajectory is identical
...
def train_loop(config: Config):
    ...
    with mesh:
        ...
        # Gradient-norm diagnostic: per-step global grad norm written to a
        # standalone JSON file (master process only). Kept fully separate from
        # the existing logs so it has no effect on training or other logging.
        grad_norm_records = []
        grad_norm_path = (
