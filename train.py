...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. The multiplier ramps linearly
    # from 0 to the peak (1.0) over the warmup, holds flat at the peak through
    # the stable phase, then decays linearly from the peak down to 0 over the
    # warmdown. Peak multiplier is 1.0, so peak LR (== base_lr) is unchanged.
