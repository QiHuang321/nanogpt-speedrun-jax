...
class Config:
    ...
    n_warmup_iters: int = 100  # warmup-stable-decay: linear LR warmup steps
    f_warmdown_iters: float = 0.2  # warmup-stable-decay: decay over final fraction
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # the base LR. The peak multiplier is 1.0, so the peak LR is unchanged:
    #   1) linear warmup   0 -> 1   over the first n_warmup_iters steps
    #   2) stable phase     1        through the middle of training
    #   3) linear decay    1 -> 0    over the final n_warmdown_iters steps
    # jnp.maximum(..., 1) guards the disabled-phase (zero-length) cases against
    # division by zero; those branches are masked out by the jnp.where anyway.
