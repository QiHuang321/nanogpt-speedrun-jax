...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding: gpt_forward reuses wte
    # (transposed) as the output projection instead of a separate weight.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    # so embedding and lm_head share a single weight matrix.
