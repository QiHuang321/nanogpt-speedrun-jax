...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. Peak LR multiplier is 1.0 so
    # per-group base (peak) LRs are unchanged: linear warmup 0 -> 1, a stable
    # plateau at 1, then linear decay 1 -> 0 over the final n_warmdown_iters.
