...
class Config:
    ...
    # muon_ns_iters over the first muon_ns_warmup_frac of training (fewer early).
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (step-dependent) count, so use a dynamic loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # over the first ns_warmup_frac of training (fewer iterations early).
        ns_frac = jnp.minimum(step / jnp.maximum(ns_warmup_frac * n_train_iters, 1.0), 1.0)
