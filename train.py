...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # (callers scale this by their own base_lr, so the peak LR is unchanged):
    #   1) linear warmup from 0 -> 1 over the first n_warmup_iters steps,
    #   2) a stable plateau held at the peak of 1.0 through the middle,
    #   3) linear decay from 1 -> 0 over the final n_warmdown_iters steps.
    # jnp.maximum(..., 1) guards the (masked) branches against divide-by-zero
    # when warmup or warmdown is disabled.
