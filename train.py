...
class Config:
    ...
    muon_ns_iters: int = 5
    muon_ns_iters_min: int = 2  # fewer Newton-Schulz iters early, ramping up to muon_ns_iters
    muon_eps: float = 1e-7
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Newton-Schulz iteration count ramps linearly from ns_iters_min (early,
        # when grads are noisy) up to the full ns_iters by the end of training.
        progress = (step + 1) / n_train_iters
        ns_steps = jnp.floor(
            ns_iters_min + (ns_iters - ns_iters_min) * progress
        ).astype(jnp.int32)
        ns_steps = jnp.clip(ns_steps, ns_iters_min, ns_iters)
