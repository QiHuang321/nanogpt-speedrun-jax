...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the output projection shares the token embedding
    # (params["wte"], transposed in gpt_forward), so there is no separate
    # lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token embedding weights (transposed).
