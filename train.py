...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # tied weights: the lm_head reuses the token embedding (wte) transposed,
    # so no separate output-projection parameter is allocated here.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # tied weights: unembed with the transpose of the token embedding (wte)
