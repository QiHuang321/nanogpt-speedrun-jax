...
class Config:
    ...
    f_warmdown_iters: float = 0.2  # warmup-stable-decay: final 20% linear decay
    ...
    muon_ns_iters: int = 2  # adaptive NS: full iteration count by end of training
    muon_ns_iters_min: int = 1  # adaptive NS: fewer iterations early in training
    muon_eps: float = 1e-7
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoidal) schedule, returned as a multiplier on
    # the peak LR (peak == 1.0, so the configured base LRs are unchanged):
    #   1) linear warmup  0.0 -> 1.0 over the first n_warmup_iters steps
    #   2) stable phase    held at the peak (1.0)
    #   3) linear decay   1.0 -> 0.0 over the final n_warmdown_iters steps
    # jnp.maximum guards the denominators so a zero-length warmup or decay
    # phase degrades gracefully to a constant schedule instead of dividing by 0.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced scalar (adaptive NS count that grows with the
        # training step), so use a dynamic-trip-count fori_loop instead of a
        # static Python range() unroll.
...
def get_ns_iters(step, ns_iters_min, ns_iters_max, n_train_iters):
    # Adaptive Newton-Schulz iteration count: use fewer NS iterations early in
    # training (when the momentum buffer is noisy and a coarse orthogonalization
    # is sufficient) and ramp up linearly to the full count by the end of the
    # run. Returned as a traced int32 so it can drive a dynamic-trip-count
    # fori_loop inside zeropower_via_newtonschulz5.
    frac = jnp.clip(step / jnp.maximum(n_train_iters, 1), 0.0, 1.0)
    ns_iters = ns_iters_min + jnp.floor(frac * (ns_iters_max - ns_iters_min + 1))
    return jnp.clip(ns_iters, ns_iters_min, ns_iters_max).astype(jnp.int32)
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding matrix (wte) rather
    # than owning a separate (d_model, vocab_size) parameter. gpt_forward uses
    # wte.T for the output projection, so there is no "lm_head" key to init.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token-embedding matrix (transposed) as the output
    # projection instead of a separate lm_head parameter.
