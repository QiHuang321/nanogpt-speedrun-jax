...
from jax.nn import initializers, relu, gelu, log_softmax
...
class Config:
    ...
    # first n_warmup_iters steps, then a stable phase held at the peak LR, then
    # linear decay to ~0 over the final f_warmdown_iters fraction. n_warmup_iters
    # is a fixed 100-step ramp and n_warmdown_iters is derived from its fraction,
    # both set in __post_init__. (f_warmup_iters is retained but no longer used.)
    ...
    muon_ns_iters: int = 2  # handicap; max (final) Newton-Schulz iters
    muon_ns_iters_min: int = 1  # min (early) Newton-Schulz iters; ramps up to muon_ns_iters
    muon_eps: float = 1e-7
    ...
    def __post_init__(self):
        ...
        # Warmup is a fixed 100-step linear ramp (absolute step count, not a
        # fraction of the run) so the warmup branch in get_lr is live.
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # peak (base) LR so the peak itself is left unchanged. Linear warmup from 0
    # up to the peak (1.0) over the first n_warmup_iters steps, a stable phase
    # held at the peak, then linear decay from the peak down to ~0 over the final
...
def get_ns_iters(step, ns_iters_min, ns_iters_max, n_train_iters):
    # adaptive Newton-Schulz iteration count: fewer early, ramping up over training
    frac = jnp.clip(step / n_train_iters, 0.0, 1.0)
    val = ns_iters_min + frac * (ns_iters_max - ns_iters_min)
    return jnp.clip(jnp.round(val).astype(jnp.int32), ns_iters_min, ns_iters_max)
