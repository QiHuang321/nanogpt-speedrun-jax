...
class Config:
    ...
    # Intermediate evaluation points, given as fractions of the run, at which to
    # run an extra (observational) validation pass on top of the val_loss_every
    # cadence. Monitoring only — these do not touch params/opt_state, so the
    # training path is unchanged.
    eval_milestones: tuple[float, ...] = (0.25, 0.5, 0.75)
    ...
    # muon for matrices
    muon_base_lr: float = 0.06
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    muon_ns_iters: int = 2  # handicap (final Newton-Schulz iters, reached late in training)
    muon_ns_iters_init: int = 1  # fewer Newton-Schulz iters early in training
    muon_eps: float = 1e-7

    # adam for non-matrices
    adam_nonmat_base_lr: float = 0.08  # handicap
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop (vs a Python range) lets `steps` be a traced value, so the
        # iteration count can vary with the training step (see muon()).
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Exponential approach toward the final momentum, with a time constant
        # of momentum_warmup_steps (reaches ~63% of the way at one step, ~95% at
        # three), rather than ramping linearly to the final value.
        momentum = warmup_momentum_final + (
            warmup_momentum_init - warmup_momentum_final
        ) * jnp.exp(-step / momentum_warmup_steps)
        # Adaptive Newton-Schulz iteration count: ramp linearly from
        # ns_iters_init (fewer, early) to ns_iters (final, late in training).
        ns_frac = jnp.clip(step / n_train_iters, 0.0, 1.0)
        ns_iters_now = jnp.round(
            ns_iters_init + ns_frac * (ns_iters - ns_iters_init)
        ).astype(jnp.int32)
        new_m = tree_map(
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, ns_iters_now, eps).astype(
                    p.dtype
                )
...
def train_loop(config: Config):
    ...
    with mesh:
        ...
        # Resolve fractional eval milestones to integer step indices. These give
        # extra observational validation points at fixed progress fractions,
        # independent of the val_loss_every cadence. Steps that also land on the
        # regular cadence are handled by the existing eval block (via elif), and
        # the 0.0/1.0 endpoints are skipped (start has no signal; the end gets a
        # dedicated final validation).
        eval_milestone_steps = {
            int(f * config.n_train_iters)
            for f in config.eval_milestones
            if 0.0 < f < 1.0
        }
        ...
        for step in range(config.n_train_iters):
            ...
            if step > 0 and (step % config.val_loss_every == 0):
                ...
                if (
                    ...
                    break
            elif step in eval_milestone_steps:
                # Intermediate evaluation point: an observational validation pass
                # at a configured progress milestone, on top of the regular
                # val_loss_every cadence. Read-only w.r.t. params/opt_state and
                # with no early-stop, so the training path is unchanged.
                run_evaluation(
                    step,
                    config,
                    params,
