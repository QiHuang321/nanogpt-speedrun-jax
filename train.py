...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses the token embedding (wte) rather
    # than carrying its own lm_head parameter. See gpt_forward, which projects
    # the final hidden state through params["wte"].T.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding as the output projection. wte is
    # (vocab_size, d_model); its transpose (d_model, vocab_size) matches the
    # (i, o) layout linear() expects.
