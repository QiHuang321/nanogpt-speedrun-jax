...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # matrix `wte` (used transposed in gpt_forward), so no separate lm_head
    # parameter is allocated. The adam_lm_head optimizer simply has no leaves to
    # act on. To untie, restore:
    #     params["lm_head"] = sharded_zeros((config.d_model, config.vocab_size))
    # and use params["lm_head"] in gpt_forward instead of params["wte"].T.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied output projection (shares wte)
