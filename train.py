...
class Config:
    ...
    muon_ns_iters: int = 5
    # adaptive Newton-Schulz: ramp iteration count from muon_ns_iters_min up
    # to muon_ns_iters over the first muon_ns_warmup_frac of training
    muon_ns_iters_min: int = 3
    muon_ns_warmup_frac: float = 0.25
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced scalar (adaptive to the training step),
        # so iterate with fori_loop rather than a Python loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Early gradients don't need a precise orthogonalization: ramp the
        # Newton-Schulz iteration count from ns_iters_min to ns_iters over
        # the first ns_warmup_frac of training.
        ns_warmup_steps = max(int(n_train_iters * ns_warmup_frac), 1)
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to wte (see gpt_forward), so no separate parameter here.
