...
class Config:
    ...
    f_warmdown_iters: float = 0.4  # warmup-stable-decay: decay over final 40%
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # [0, 1] applied to each optimizer's base (peak) LR, so the peak LR is
    # unchanged. Three phases:
    #   1) linear warmup from 0 -> 1 over the first n_warmup_iters steps,
    #   2) stable at the peak (1.0),
