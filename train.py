...
class Config:
    ...
    muon_ns_iters: int = 5
    muon_ns_iters_min: int = 2  # NS iters early in training; ramps up to muon_ns_iters
    muon_eps: float = 1e-7

    # adam for non-matrices
    adam_nonmat_base_lr: float = 0.05333333333333334  # handicap
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (per-training-step) value, so use fori_loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: ramp linearly from ns_iters_min
        # early in training up to ns_iters by the final step (fewer iters early).
        ns_frac = jnp.clip(step.astype(jnp.float32) / n_train_iters, 0.0, 1.0)
        ns_iters_step = ns_iters_min + jnp.round(
