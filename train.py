...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # peak LR (the peak is 1.0, so each optimizer's peak base_lr is unchanged):
    #   * warmup: linear ramp 0 -> 1 over the first n_warmup_iters steps
    #   * stable: held constant at the peak of 1.0
    #   * decay:  linear ramp 1 -> 0 over the final n_warmdown_iters steps
    # jnp.maximum guards the divisors so a zero-length warmup/decay phase (e.g.
    # n_warmup_iters == 0) does not produce inf in the unselected branch.
