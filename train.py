...
from jax.nn import initializers, log_softmax
...
class Config:
    ...
    n_warmup_iters: int = 100  # WSD warmup: linear ramp to peak LR
    f_warmdown_iters: float = 0.2  # WSD decay fraction: linear ramp peak -> 0
    ...
    adam_nonmat_base_lr: float = 0.0237  # handicap
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) multiplier on the peak (base) LR:
    #   - linear warmup 0 -> 1 over the first n_warmup_iters steps,
    #   - constant plateau at 1.0 (peak LR unchanged) through the stable phase,
...
def train_step(
    ...
    # optimizer. Computed in float32 over all leaves; does not affect the
    # update below, so the training trajectory is unchanged.
    grad_norm = jnp.sqrt(
        sum(
...
def train_loop(config: Config):
    ...
    # Gradient-norm diagnostic hook: append per-step global grad norms to a
    # standalone JSON file (master process only). Purely observational and kept
    # out of the Logger's logs/ output; rewritten each step so partial results
    # survive a crash.
    grad_norm_path = (
        ...
        for step in range(config.n_train_iters):
            ...
            if grad_norm_path is not None:
                grad_norm_records.append(
                    {"step": step, "grad_norm": grad_norm_val}
                )
