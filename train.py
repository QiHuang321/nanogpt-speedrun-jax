...
class Config:
    ...
    # warmup-stable-decay (trapezoid) schedule: linear warmup, flat peak, linear
    # decay to 0 over the final f_warmdown_iters fraction of training.
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # that each optimizer's base_lr stays the (unchanged) peak learning rate:
    #   - warmup: linear ramp 0 -> 1 over the first n_warmup_iters steps
    #   - stable: held flat at 1.0
    #   - decay:  linear ramp 1 -> 0 over the final n_warmdown_iters steps
    # jnp.maximum(., 1) guards the zero-length-phase case (no div-by-zero); the
    # corresponding branch is masked out by the jnp.where below anyway.
