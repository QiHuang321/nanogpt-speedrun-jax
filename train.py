...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # matrix instead of being a separate parameter. The transpose is applied at
    # use time in gpt_forward, so no distinct "lm_head" leaf is created here.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token-embedding matrix (transposed) as lm_head.
