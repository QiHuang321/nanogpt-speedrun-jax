...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the lm_head reuses the token embedding (wte) transposed, so
    # there is no standalone lm_head parameter. The lm_head logits are computed
    # in gpt_forward as x @ wte.T.
    ...
    for i in range(config.n_layers):
        ...
        # Small-scale normal init for the attention output projection
        # (replaces zero-init) so the residual branch starts with a small but
        ...
        # Small-scale normal init for the MLP output projection
        # (replaces zero-init), matching the attention output projection above.
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    q = rms_norm(q, config)  # QK-norm: RMSNorm over the head dim before RoPE
    k = rms_norm(k, config)
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: project with the transposed token embedding (wte) weights.
