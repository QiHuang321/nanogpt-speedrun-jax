...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is annealed from muon_ns_iters_init (early)
    # up to muon_ns_iters over training: fewer iterations early, more later.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Anneal the Newton-Schulz iteration count from ns_iters_init up to
        # ns_iters over training: fewer iterations early, more later.
        ns_frac = jnp.minimum(step / n_train_iters, 1.0)
        ns_steps = ns_iters_init + jnp.round(
