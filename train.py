...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # than owning a separate parameter. The output projection is computed as
    # wte.T in gpt_forward, so there is no standalone "lm_head" leaf to init.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied: lm_head == token-embedding (wte)
