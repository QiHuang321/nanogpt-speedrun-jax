...
class Config:
    ...
    muon_ns_iters_init: int = 1  # Newton-Schulz iters early in training (ramps up to muon_ns_iters)
    muon_eps: float = 1e-7
...
def get_ns_iters(it, ns_iters_init, ns_iters_final, n_train_iters):
    # Adaptive Newton-Schulz iteration count: use fewer iterations early in
    # training (gradients are noisy, so a coarse orthogonalization suffices)
    # and ramp linearly up to the full count by the end of training. Returns a
    # traced int32 scalar consumed as the dynamic bound of the NS fori_loop.
    frac = jnp.clip(it / n_train_iters, 0.0, 1.0)
    iters = ns_iters_init + frac * (ns_iters_final - ns_iters_init)
    return jnp.maximum(jnp.round(iters), 1).astype(jnp.int32)
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced int (adaptive per training step), so loop with
        # a dynamic bound instead of a Python-unrolled range.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        current_ns_iters = get_ns_iters(step, ns_iters_init, ns_iters, n_train_iters)
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, current_ns_iters, eps).astype(p.dtype)
