...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # (transposed) instead of a separate lm_head tensor, so wte and the lm_head
    # share a single weight that receives gradients from both. (To untie, restore
    # `params["lm_head"] = sharded_zeros((config.d_model, config.vocab_size))`
    # and use `linear(x, params["lm_head"])` in gpt_forward.)
