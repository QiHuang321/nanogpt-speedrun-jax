...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a dynamic (traced) iteration count, so use fori_loop
...
def muon(
    ...
    # Newton-Schulz iteration count ramps from ns_iters_min up to ns_iters over
    # the first half of training, so early (noisy) steps use fewer iterations.
    ns_ramp_iters = max(1, n_train_iters // 2)
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output head reuses the (transposed) token embedding, so
    # there is no separate "lm_head" parameter (see gpt_forward).
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding (transposed) as the output head.
