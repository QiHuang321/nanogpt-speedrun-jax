...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embeddings: the output projection reuses params["wte"] (transposed)
    # in gpt_forward, so we no longer allocate a separate lm_head weight.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding (transposed) as the output
