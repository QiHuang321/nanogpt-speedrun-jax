...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # matrix (wte), so no separate lm_head parameter is allocated. See
    # gpt_forward, which uses wte^T as the head.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: reuse the token-embedding matrix (wte) as the output
    # projection (wte^T) instead of a separate lm_head parameter.
