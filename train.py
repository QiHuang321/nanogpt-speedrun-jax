...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iterations ramp from ns_iters_init up to ns_iters over
    # ns_iters_warmup_steps (fewer iterations early in training).
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (dynamic) value, so use fori_loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp Newton-Schulz iterations from ns_iters_init up to ns_iters
        # (fewer iterations early in training).
