...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.03375  # handicap
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    muon_ns_iters: int = 5
    # Newton-Schulz iterations are ramped from a smaller count early in training
    # up to muon_ns_iters, linearly over muon_ns_iters_warmup_steps steps.
    muon_ns_iters_init: int = 2
    muon_ns_iters_warmup_steps: int = 500
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop (rather than a Python range) so `steps` may be a traced
        # value that varies with the training step (see muon's adaptive count).
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz count: ramp linearly from ns_iters_init up to
        # ns_iters over ns_iters_warmup_steps, so early steps run fewer (cheaper,
        # less precise) iterations. Held at ns_iters thereafter.
        ns_frac = jnp.minimum(step / max(ns_iters_warmup_steps, 1), 1.0)
