...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding matrix (no separate
    # lm_head parameter). gpt_forward uses params["wte"].T for the output proj.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: output projection reuses the token-embedding matrix.
