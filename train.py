...
class Config:
    ...
    muon_ns_iters: int = 2
    # Adaptive Newton-Schulz: ramp the iteration count from a smaller value
    # early in training up to muon_ns_iters, linearly over the first
    # muon_ns_iters_warmup_steps steps. Fewer iterations early cuts the
    # orthogonalization compute while updates are coarse anyway; the full count
    # kicks in once cleaner updates start to matter.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop (not a Python range) so `steps` may be a traced value: the
        # Muon Newton-Schulz iteration count is scheduled per training step
        # (fewer early). A dynamic bound lowers to lax.while_loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp the Newton-Schulz iteration count from ns_iters_init up to
        # ns_iters over the first ns_iters_warmup_steps steps (fewer early).
