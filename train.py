...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # applied to each optimizer's peak (base) LR; the peak value is unchanged.
    #   1) linear warmup 0 -> 1 over the first n_warmup_iters steps
    #   2) stable phase held flat at the peak (1.0)
    #   3) linear decay 1 -> 0 over the final n_warmdown_iters steps
    # max(..., 1) guards the degenerate (zero-length) warmup/decay phases, which
    # are then never selected by the jnp.where below.
