...
class Config:
    ...
    adam_nonmat_base_lr: float = 0.08 * 2 / 3
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # wte (as wte.T) instead of a separate parameter (see gpt_forward).
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # tied output projection: reuse the token embedding (wte) as lm_head
