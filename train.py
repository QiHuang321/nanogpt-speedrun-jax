...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: lm_head shares the token-embedding matrix (used as wte.T in
    # gpt_forward), so no separate lm_head parameter is allocated.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: project with the transpose of the token embedding (wte.T).
