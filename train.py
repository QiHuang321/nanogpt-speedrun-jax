...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iterations ramp from ns_iters_start (early) up to muon_ns_iters
    # (final) over training -- fewer iters early when gradients are noisy.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters_start up to ns_iters over the course of training (fewer early).
        ns_frac = jnp.minimum(step / n_train_iters, 1.0)
        ns_iters_now = jnp.floor(
            ns_iters_start + ns_frac * (ns_iters - ns_iters_start) + 0.5
        ).astype(jnp.int32)
        ns_iters_now = jnp.clip(ns_iters_now, ns_iters_start, ns_iters)
        new_m = tree_map(
