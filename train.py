...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # params["wte"].T instead of a separate weight (see gpt_forward).
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token embedding weights (wte) instead
    # of a separate lm_head matrix.
