...
class Config:
    ...
    muon_ns_iters: int = 2  # handicap; final (late-training) Newton-Schulz iters
    muon_ns_iters_min: int = 1  # fewer Newton-Schulz iters early in training
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-step adaptive) value, so loop dynamically.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp Newton-Schulz iters from `ns_iters_min` (early) up to `ns_iters`
        # (late) linearly with training progress: cheaper, looser orthogonalization
        # early when gradients are noisy, tightening as training proceeds.
        progress = jnp.clip(step / n_train_iters, 0.0, 1.0)
        ns_iters_step = ns_iters_min + (
            progress * (ns_iters - ns_iters_min + 1)
        ).astype(jnp.int32)
        ns_iters_step = jnp.clip(ns_iters_step, ns_iters_min, ns_iters)
