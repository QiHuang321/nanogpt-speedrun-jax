...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped up over training: use
    # muon_ns_iters_init iterations early (cheaper, gradients are noisy anyway)
    # and increase linearly to muon_ns_iters over muon_ns_warmup_steps steps.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # loop rather than a Python-level unrolled range().
        def _ns_body(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # training, ramping linearly up to ns_iters over ns_warmup_steps.
        ns_frac = jnp.minimum(step / ns_warmup_steps, 1.0)
        ns_iters_now = ns_iters_init + jnp.floor(
