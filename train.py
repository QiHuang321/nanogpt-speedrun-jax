...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    # over the first n_warmup_iters, hold at the peak (multiplier 1.0) through
    # the stable phase, then linearly decay to 0 over the final
    # n_warmdown_iters. Peak LR (== base_lr) is unchanged from before.
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection (lm_head) shares the token-embedding
    # matrix (wte) instead of being a separate parameter. gpt_forward uses wte.T
    # for the final logit projection, so gradients accumulate into wte from both
    # the embedding lookup and the output projection.
