...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.015  # handicap: quartered from tuned 0.04 (s0 headroom)
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    muon_ns_iters: int = 5
    # Adaptive Newton-Schulz: use fewer NS iterations early in training (when
    # the momentum buffer is noisy and gradients are large, precise
    # orthogonalization buys little) and ramp linearly up to muon_ns_iters over
    # the first muon_ns_warmup_steps steps. Past the warmup, behavior is
    # identical to a fixed muon_ns_iters run.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced int (adaptive per-step schedule), so use a
        # dynamic-bound fori_loop rather than a Python range() unroll.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: ramp from ns_iters_min up to
        # ns_iters over the first ns_warmup_steps steps (fewer iterations early,
        # full precision once the optimizer has warmed up).
        ns_frac = jnp.minimum(step / max(ns_warmup_steps, 1), 1.0)
        ns_steps = jnp.round(
            ns_iters_min + ns_frac * (ns_iters - ns_iters_min)
        ).astype(jnp.int32)
        new_m = tree_map(
