...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the token embedding `wte` doubles as the output projection
    # (see gpt_forward, which uses `wte.T` as the lm_head), so there is no
    # separate `lm_head` parameter to allocate.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied output projection: reuse the token embedding (wte.T) as the lm_head.
