...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (step-dependent) value, so use fori_loop
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # fewer Newton-Schulz iterations early in training, ramping up to
        # the full count over ns_iters_warmup_steps
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    q, k = rms_norm(q, config), rms_norm(k, config)
