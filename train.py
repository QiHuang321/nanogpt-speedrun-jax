...
class Config:
    ...
    adam_nonmat_base_lr: float = 0.04 * 2 / 3 * 2 / 3 * 2 / 3 * 2 / 3
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding matrix (used
    # transposed in gpt_forward), so there is no separate lm_head parameter.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: project with the transposed token-embedding matrix instead
    # of a dedicated lm_head, so wte receives gradients from both paths.
