...
class Config:
    ...
    f_warmdown_iters: float = 0.2  # fraction of training spent decaying LR to 0
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule: linearly warm up from 0 to the
    # peak over the first n_warmup_iters steps, hold at the (unchanged) peak
    # during the stable phase, then linearly decay back down to 0 over the
