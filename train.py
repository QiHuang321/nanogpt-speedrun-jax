...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head weights are tied to the token embedding (wte); no separate
    # parameter is created. See gpt_forward, which uses wte.T for the logits.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: reuse the token embedding (wte) transposed as the output
    # projection, so logits[..., v] = x . wte[v].
