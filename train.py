...
class Config:
    ...
    # warmup-stable-decay (trapezoid) LR schedule: linear warmup, then a
    # stable phase at peak LR, then a linear decay over the final fraction of
    # training. Peak LR is set per-optimizer-group base_lr and is unchanged.
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to the token embedding (wte.T), so no separate param
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied lm_head: weights shared with wte
