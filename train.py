...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. The returned value is a
    # multiplier on each optimizer's base_lr and peaks at 1.0, so the peak LR
    # is unchanged. Linear warmup up to the peak, a constant (stable) phase
    # held at the peak, then a linear decay from the peak down to 0 over the
    # final n_warmdown_iters steps.
