...
class Config:
    ...
    # muon_ns_iters over the first muon_ns_iters_warmup_steps steps, so early
    # (noisy) gradients get a cheaper, coarser orthogonalization.
    muon_ns_iters_init: int = 3
    muon_ns_iters_warmup_steps: int = 300
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced value (adaptive per training step), so loop with
        # fori_loop rather than a static Python range.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # ns_iters_init up to ns_iters over the first ns_iters_warmup_steps
        # steps, so early steps use fewer (cheaper) iterations.
        ns_warmup = max(ns_iters_warmup_steps, 1)
        ns_iters_now = ns_iters_init + ((ns_iters - ns_iters_init) * step) // ns_warmup
        ns_iters_now = jnp.minimum(ns_iters_now, ns_iters)
...
def init_optimizer(config: Config, params: PyTree, mesh: Mesh):
    ...
    muon_opt = muon(
        ...
        config.muon_ns_iters_warmup_steps,
        config.muon_eps,
    )
