...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    ...
    #   * stable at 1 (peak) through the middle,
    #   * linear decay   1 -> 0 over the final n_warmdown_iters steps.
    # jnp.maximum guards keep zero-length warmup/decay phases division-safe.
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to wte (see gpt_forward); no separate output-projection
    # weight is allocated, so its gradient flows back into wte directly.
...
def attention_forward(params, x, v1, cos, sin, config):
    ...
    q, k = rms_norm(q, config), rms_norm(k, config)  # QK-norm over head dim
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied embeddings: reuse the token embedding matrix as the output
    # projection (lm_head = wte.T). wte is (vocab, d_model), so transpose to
    # (d_model, vocab) for the logit projection.
