...
class Config:
    ...
    muon_ns_iters: int = 2
    # Adaptive Newton-Schulz iteration count (fewer early): linearly ramp the
    # number of NS orthogonalization iterations from muon_ns_iters_start up to
    # muon_ns_iters over the first muon_ns_iters_warmup_steps optimizer steps.
    # Early in training the momentum buffer is still noisy, so fewer iterations
    # are sufficient (and cheaper); the full count kicks in once it settles.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (per-step adaptive) iteration count, so use a
        # fori_loop with a dynamic trip count rather than a Python range unroll.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count (fewer early): linearly ramp
        # from ns_iters_start up to ns_iters over the first ns_iters_warmup_steps
        # steps, clamped to at least one iteration so the update always stays
        # orthogonalized.
        ns_frac = jnp.minimum(step / ns_iters_warmup_steps, 1.0)
        cur_ns_iters = jnp.maximum(
            ns_iters_start
            + jnp.floor(ns_frac * (ns_iters - ns_iters_start)).astype(jnp.int32),
            1,
        )
        new_m = tree_map(
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, cur_ns_iters, eps).astype(p.dtype)
