...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # (wte) rather than being a separate parameter. The logits are computed
    # with wte.T in gpt_forward, so no standalone lm_head weight is allocated.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: project with the transpose of the token-embedding matrix.
