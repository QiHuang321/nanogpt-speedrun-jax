...
class Config:
    ...
    # f_warmdown_iters in __post_init__ (decay over the final 20% of training).
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # tied weights: lm_head shares the token embedding (wte), so no separate param
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied: lm_head shares the token embedding
    c = config.logit_softcap
    logits = 2.0 * c * jax.nn.sigmoid(logits / (c / 2.0))  # tanh-style logit softcap
