...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # parameter; gpt_forward reuses wte.T for the output projection, so a single
    # weight is shared (and receives gradients from) both the input and output.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied lm_head: reuse token embedding
