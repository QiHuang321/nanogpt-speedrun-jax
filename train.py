...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the output projection reuses the token embedding (wte.T) in
    # gpt_forward, so there is no separate lm_head weight to initialize.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding (wte) as the output projection.
