...
class Config:
    ...
    # Warmup-stable-decay (trapezoid) LR schedule: linear warmup, hold at peak,
    # then linear decay over the final f_warmdown_iters fraction of training.
    # Peak LR is unchanged (get_lr multiplier stays 1.0 on the plateau); this
    # only enables the decay tail (n_warmdown_iters derived in __post_init__).
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embeddings: the output projection (lm_head) shares the token
    # embedding matrix (wte), so there is no separate lm_head parameter. The
    # unembedding is computed as wte.T in gpt_forward.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied embeddings: reuse the token embedding (wte) as the unembedding
    # instead of a separate lm_head weight. wte is (vocab, d_model), so its
    # transpose (d_model, vocab) is the output projection linear expects.
