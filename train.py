...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the lm_head shares the token embedding (wte) rather than
    # owning a separate (d_model, vocab_size) matrix. The output projection in
    # gpt_forward uses wte.T, so wte receives gradients from both the embedding
    # lookup and the unembedding and is optimized by adam_embed.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    logits = linear(x, params["wte"].T)  # tied weights: lm_head == token embedding
