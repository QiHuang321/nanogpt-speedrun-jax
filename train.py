...
class Config:
    ...
    muon_ns_iters: int = 5
    muon_ns_iters_init: int = 3  # fewer Newton-Schulz iters early in training
    muon_eps: float = 1e-7
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-step) value, so use a dynamic loop
        # rather than a Python `for range(steps)` that unrolls at trace time.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adapt the Newton-Schulz iteration count to the training step: use fewer
        # iterations early (cheaper, and early gradients are coarse) and ramp
        # linearly up to the full `ns_iters` count as training progresses.
        ns_progress = jnp.clip(step / n_train_iters, 0.0, 1.0)
        ns_iters_now = jnp.round(
            ns_iters_init + (ns_iters - ns_iters_init) * ns_progress
        ).astype(jnp.int32)
        ns_iters_now = jnp.clip(ns_iters_now, ns_iters_init, ns_iters)
        new_m = tree_map(
