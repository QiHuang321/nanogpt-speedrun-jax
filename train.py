...
class Config:
    ...
    # Newton-Schulz iteration count is annealed over training: it starts at
    # muon_ns_min_iters and ramps up to muon_ns_iters once training has
    # progressed muon_ns_warmup_frac of the way through. Cheaper, coarser
    # orthogonalization early (when gradients are noisy) saves compute, with
    # full-precision orthogonalization later.
    muon_ns_iters: int = 2
    muon_ns_min_iters: int = 1
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # fori_loop supports a traced `steps`, so the iteration count can be
        # made step-dependent without changing array shapes (compiles to a
        # while_loop when `steps` is dynamic).
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Anneal the Newton-Schulz iteration count from ns_min_iters up to
        # ns_iters over the first ns_warmup_frac of training (fewer early).
        ns_progress = jnp.minimum(
            step / jnp.maximum(ns_warmup_frac * n_train_iters, 1.0), 1.0
        )
        cur_ns_iters = ns_min_iters + jnp.round(
            ns_progress * (ns_iters - ns_min_iters)
        ).astype(jnp.int32)
        new_m = tree_map(
