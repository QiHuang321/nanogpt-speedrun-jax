...
class Config:
    ...
    # muon for matrices
    muon_base_lr: float = 0.015  # handicap
    muon_momentum_warmup_steps: int = 500
    muon_warmup_momentum_init: float = 0.85
    muon_warmup_momentum_final: float = 0.95
    muon_ns_iters: int = 5
    # Adaptive Newton-Schulz iteration count: use `muon_ns_iters_init`
    # iterations at the start of training and linearly ramp up to
    # `muon_ns_iters` over `muon_ns_warmup_steps` steps.
    muon_ns_iters_init: int = 2
    muon_ns_warmup_steps: int = 400
    muon_eps: float = 1e-7
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule. Returns a multiplier in [0, 1]
    # applied to each optimizer's base (peak) LR, so the peak LR is unchanged.
    warmup_lr = (it + 1) / n_warmup_iters  # linear warmup: 0 -> 1
    constant_lr = 1.0  # stable phase: hold at peak
    warmdown_lr = (n_train_iters - it) / n_warmdown_iters  # linear decay: 1 -> 0
    lr = jnp.where(
        it < n_warmup_iters,
        warmup_lr,
...
def get_ns_iters(step, ns_iters_init, ns_iters_final, ns_warmup_steps):
    # Adaptive Newton-Schulz iteration count. Orthogonalization need not be
    # precise while gradients are still noisy early in training, so use fewer
    # NS iterations at the start and linearly ramp up to the full count over
    # `ns_warmup_steps`. Returns a traced int32 trip count in
    # [ns_iters_init, ns_iters_final].
    frac = jnp.minimum((step + 1) / ns_warmup_steps, 1.0)
    n = ns_iters_init + frac * (ns_iters_final - ns_iters_init)
    n = jnp.round(n).astype(jnp.int32)
    return jnp.clip(n, ns_iters_init, ns_iters_final)
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced scalar (adaptive per-step iteration count);
        # fori_loop supports a dynamic upper bound under jit.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        ns_iters_step = get_ns_iters(step, ns_iters_init, ns_iters, ns_warmup_steps)
        new_m = tree_map(
        ...
        def _update_leaf(g, p, m):
            ...
            update = (
                lr.astype(p.dtype)
                * zeropower_via_newtonschulz5(g_nesterov, ns_iters_step, eps).astype(
                    p.dtype
                )
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the lm_head reuses the token embedding (wte) instead of
    # being a separate parameter, so it is not initialized here. The output
    # projection uses wte.T in gpt_forward.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: project with the (transposed) token embedding (wte) rather
    # than a separate lm_head parameter.
