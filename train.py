...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the LM head reuses the token-embedding matrix (wte.T) as the
    # output projection, so there is no separate lm_head parameter. The
    # adam_lm_head optimizer therefore receives an empty subset and no-ops.
