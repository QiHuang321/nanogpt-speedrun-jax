...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # lm_head is tied to wte: the unembedding uses wte.T (see gpt_forward), so
    # no separate lm_head parameter is allocated.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied unembedding: reuse the token-embedding matrix (wte.T) as the lm_head.
