...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token-embedding matrix (its transpose
    # is used as the output projection in gpt_forward), so there is no separate
    # lm_head parameter to initialize.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Weight tying: reuse the token embedding (transposed) as the output head.
