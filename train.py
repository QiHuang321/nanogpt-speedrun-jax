...
class Config:
    ...
    muon_ns_iters: int = 2
    # Adaptive Newton-Schulz: use fewer iterations early in training and ramp up
    # to muon_ns_iters. ns count goes from muon_ns_iters_start to muon_ns_iters
    # linearly over the first muon_ns_iters_warmup_steps steps.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced int (adaptive Newton-Schulz schedule), so use a
        # lax loop instead of a Python range() that requires a concrete length.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: ramp from ns_iters_start up to
        # ns_iters over the first ns_iters_warmup_steps steps. Early updates use
        # fewer iterations (the momentum buffer is still noisy, so a cheaper,
        # rougher orthogonalization is enough); later updates use the full count.
