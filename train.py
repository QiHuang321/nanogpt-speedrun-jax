...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied output head: lm_head shares the token-embedding weights (wte.T is used
    # as the output projection in gpt_forward), so no separate lm_head param.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied head: reuse the token embedding (transposed) as the output projection.
