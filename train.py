...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head shares the token embedding (wte). There is no
    # separate lm_head parameter; gpt_forward projects logits with wte.T.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: project with the transpose of the token embedding (wte).
