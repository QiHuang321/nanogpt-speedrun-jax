...
class Config:
    ...
    muon_ns_iters: int = 2
    # Newton-Schulz iteration count is ramped from muon_ns_iters_min (early) up
    # to muon_ns_iters (late) over the course of training: cheaper, coarser
    # orthogonalization early on, full-precision orthogonalization later.
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # `steps` may be a traced (per-step) value, so use a rolled fori_loop
        # with a dynamic trip count rather than a Python-unrolled range loop.
        def _ns_step(i, X):
            A = X @ X.T
            B = b * A + c * (A @ A)
            return a * X + B @ X
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Ramp the Newton-Schulz iteration count from ns_iters_min up to ns_iters
        # over training: fewer (cheaper, coarser) iterations early on.
