...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Weight tying: the output projection (lm_head) shares the token-embedding
    # weights, so there is no separate lm_head parameter. gpt_forward uses
    # wte.T for the logits; autodiff accumulates grads into wte from both uses.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied weights: reuse the token embedding as the output projection (wte.T).
