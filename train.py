...
class Logger:
    def __init__(self):
        ...
        # Gradient-norm diagnostic: written to its own JSON file (in the cwd,
        # not under logs/ or records/) so it is fully separate from the run log
        # and never interferes with existing logging or training state.
    ...
    def log_grad_norm(self, step: int, grad_norm: float):
        # Diagnostic-only hook: append the global gradient L2 norm for this step
        # to a standalone JSON file. Does not touch params, opt_state, the data
        # cursor, the RNG, or the regular metric logging, so the training path
        # is identical whether or not this is called.
        if not self.is_master:
            return
...
class Config:
    ...
    adam_nonmat_base_lr: float = 0.04 * 2 / 3 * 2 / 3 * 2 / 3
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # warmup-stable-decay (trapezoid): linear warmup, constant at peak,
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced int (adaptive over training); fori_loop with a
        # dynamic upper bound lowers to a while loop.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: fewer iterations early in
        # training, ramping linearly up to ns_iters by the end.
        frac = jnp.clip(step / n_train_iters, 0.0, 1.0)
        ns_steps = ns_iters_init + (frac * (ns_iters - ns_iters_init + 1)).astype(
            jnp.int32
        )
        ns_steps = jnp.minimum(ns_steps, ns_iters)
...
def train_step(
    ...
    # Diagnostic only: global gradient L2 norm (accumulated in float32 to avoid
    # bf16 summation error). Returned as an extra metric; it never enters the
    # params/opt_state computation, so the training path is unchanged.
    grad_norm = jnp.sqrt(
        sum(
...
def train_loop(config: Config):
    ...
    with mesh:
        ...
        for step in range(config.n_train_iters):
            ...
            # Gradient-norm diagnostic hook: append to the standalone JSON file.
