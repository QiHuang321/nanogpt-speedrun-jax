...
class Config:
    ...
    # Warmup-stable-decay (trapezoid) edges. Peak LR is unchanged; these only
    # shape the ramp-up and ramp-down. n_warmdown_iters is derived from
    # f_warmdown_iters in __post_init__.
    ...
    adam_nonmat_base_lr: float = 0.05333333333333334
...
def get_lr(it, n_warmup_iters, n_warmdown_iters, n_train_iters):
    # Warmup-stable-decay (trapezoid) schedule, expressed as a multiplier on the
    # (unchanged) peak learning rate:
    #   * linear warmup  0 -> 1 over the first n_warmup_iters steps
    #   * stable phase   held flat at the peak (1.0)
    #   * linear decay   1 -> 0 over the final n_warmdown_iters steps
    # A zero-length warmup or warmdown simply collapses that edge of the
    # trapezoid (the corresponding branch is never selected). The jnp.maximum
    # guards keep the unselected branches finite in those degenerate cases.
    ...
    lr = jnp.where(
        it < n_warmup_iters,
        warmup_lr,
        jnp.where(it < decay_start, 1.0, warmdown_lr),
    )
    return lr
...
def zeropower_via_newtonschulz5(G, steps, eps):
    ...
    def _update_loop(X):
        ...
        # steps may be a traced (per-step adaptive) int, so use fori_loop
        # rather than a Python range() which would require a static count.
...
def muon(
    ...
    def update(grads, params, state):
        ...
        # Adaptive Newton-Schulz iteration count: linearly ramp from
        # ns_iters_start (fewer, used early in training) to the full ns_iters,
        # over the course of training.
        progress = jnp.minimum(step / jnp.maximum(n_train_iters, 1), 1.0)
        ns_steps = jnp.round(
            ns_iters_start + (ns_iters - ns_iters_start) * progress
        ).astype(jnp.int32)
...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied embeddings: the output projection reuses params["wte"] (see
    # gpt_forward), so there is no separate lm_head parameter.
