...
def init_params(config: Config, mesh: Mesh) -> PyTree:
    ...
    # Tied weights: the lm_head reuses the token embedding (wte) transposed, so
    # there is no separate lm_head matrix in the param tree.
...
def gpt_forward(params, idx, precomputed_params, config):
    ...
    x = rms_norm(x, config)
    # Tied lm_head: project with the transposed token embedding.
